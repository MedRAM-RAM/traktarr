...
def find_best_show_match(test_name, shows):
    ...
    for show in shows:
        ...
        print(f"Similarity scores - Name only: {similarity1:.2f}, With year: {similarity2:.2f}")
    ...
    if best_score >= 0.8:
        print(f"Final match: '{best_match['show']['title']}' ({best_match['show'].get('year', '')}) with score: {best_score:.2f}")
        return best_match
